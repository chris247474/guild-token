...
contract Curve is Storage, MerkleProver {
  ...
  {
    targetAmount_ = Swaps.originSwap(
      curve,
      _origin,
      _target,
      _originAmount,
  ...
  {
    originAmount_ = Swaps.targetSwap(
      curve,
      _origin,
      _target,
      _targetAmount,
    ...
    returns (uint256, uint256[] memory)
  {
    ...
    (uint256 curvesMinted_, uint256[] memory deposits_) = ProportionalLiquidity
    .proportionalDeposit(curve, _deposit);
  ...
  {
    return
      ProportionalLiquidity.emergencyProportionalWithdraw(curve, _curvesToBurn);
  }
    ...
    returns (uint256[] memory withdrawals_)
  {
    if (whitelistingStage) {
      whitelistedDeposited[msg.sender] = whitelistedDeposited[msg.sender].sub(
        _curvesToBurn
      );
    }
