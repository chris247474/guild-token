...
library Orchestrator {
  ...
  function getFee(Storage.Curve storage curve)
    private
    view
    returns (int128 fee_)
  {
  ...
  function includeAsset(
    ...
    if (_numeraire != _reserve)
      IERC20(_numeraire).safeApprove(_reserveApproveTo, uint256(-1));
  ...
  function includeAssimilator(
    ...
    require(
      _derivative != address(0),
      'Curve/derivative-cannot-be-zeroth-address'
    );
    ...
    require(
      _assimilator != address(0),
      'Curve/assimilator-cannot-be-zeroth-address'
    );

    IERC20(_numeraire).safeApprove(_derivativeApproveTo, uint256(-1));

    Storage.Assimilator storage _numeraireAssim = curve.assimilators[
      _numeraire
    ];

    curve.assimilators[_derivative] = Storage.Assimilator(
      _assimilator,
      _numeraireAssim.ix
    );
