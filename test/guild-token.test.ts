...
describe('Guild Token', function () {
  before(async () => {
    ...
    const HaloTokenContract = await ethers.getContractFactory('GuildToken')
  ...
  it('HaloToken should be deployed', async () => {
    const expectedTotalSupply = await haloTokenContract.balanceOf(owner.address)
    expect(await haloTokenContract.symbol()).to.equal('GUILD')
    expect(await haloTokenContract.name()).to.equal('Guild Token')
